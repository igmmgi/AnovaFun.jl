...
function _normalize_sim_inputs(
    ...
    cells = _generate_cell_combinations(
        between_factors,
        within_factors,
        factor_levels,
        labelnames,
    ...
    length(mu_vec) == n_cells || throw(
        ArgumentError(
            "Number of means ($(length(mu_vec))) doesn't match design cells ($n_cells)",
        ),
    )
    length(sd_vec) == n_cells || throw(
        ArgumentError(
            "Number of sd values ($(length(sd_vec))) doesn't match design cells ($n_cells)",
        ),
    )

    r_vec = if !isempty(within_factors)
        n_within = length(
            _generate_cell_combinations(
                Symbol[],
                within_factors,
                factor_levels,
                labelnames,
        ...
        elseif r isa Number
            ...
            length(r) == n_cors_needed || throw(
                ArgumentError(
                    "r vector must have length n_within*(n_within-1)/2 = $n_cors_needed, got $(length(r))",
                ),
            )
            Float64.(r)
        end
...
"""
function _assign_subjects_to_groups(
    n::Int,
    between_factors::Vector{Symbol},
    factor_levels::Dict{Symbol,Int},
    labelnames::Dict{Symbol,Vector{String}},
    ...
    between_combinations =
        _generate_cell_combinations(between_factors, Symbol[], factor_levels, labelnames)
...
"""
function _get_subjects_for_cell(
    cell::String,
    between_factors::Vector{Symbol},
...
"""
function _generate_correlated_data(
    n::Int,
    between_factors::Vector{Symbol},
    within_factors::Vector{Symbol},
    ...
    all_factors = vcat(between_factors, within_factors)
    within_cells =
        _generate_cell_combinations(Symbol[], within_factors, factor_levels, labelnames)
    n_within = length(within_cells)

    # Generate between-subjects combinations in order to preserve level order
    between_combinations =
        isempty(between_factors) ? String[] :
        _generate_cell_combinations(between_factors, Symbol[], factor_levels, labelnames)
    ...
    if !isempty(between_factors)
        ...
        sd_vec = [
            cell_params[_combine_cell_names(first_between, cell)][2] for
            cell in within_cells
        ]
    else
        sd_vec = [cell_params[cell][2] for cell in within_cells]
    end

    all(sd_vec .> 0.0) || throw(
        ArgumentError(
            "Standard deviation (sd) must be > 0 for simulation. Got sd with values <= 0.",
        ),
    )
    ...
    factor_cols = Dict{Symbol,Vector{String}}(
        factor => Vector{String}(undef, n_rows) for factor in all_factors
    )

    # Generate data for each between-subjects group in order (or all subjects if pure within-subjects)
    groups =
        isempty(between_factors) ? [("", 1:n)] :
        [(combo, subject_assignments[combo]) for combo in between_combinations]

    row_idx = 1
    for (between_combo, subjects) in groups
        means_vec = [
            cell_params[isempty(between_combo) ? cell :
                        _combine_cell_names(between_combo, cell)][1] for
            cell in within_cells
        ]
        ...
        for subject_id in subjects
            ...
            for (i, within_cell) in enumerate(within_cells)
                full_cell =
                    isempty(between_combo) ? within_cell :
                    _combine_cell_names(between_combo, within_cell)
    ...
    return DataFrame(
        Dict(
            :id => ids,
            :dv => dvs,
            (factor => factor_cols[factor] for factor in all_factors)...,
        ),
    )
end
...
"""
function _generate_independent_data(
    cell_params::Dict{String,Vector{Float64}},
    between_factors::Vector{Symbol},
    subject_assignments::Dict{String,Vector{Int}},
    n::Int,
    all_factors::Vector{Symbol},
    factor_levels::Dict{Symbol,Int},
    labelnames::Dict{Symbol,Vector{String}},
)
    ...
    n_rows = sum(
        length(_get_subjects_for_cell(cell, between_factors, subject_assignments, n))
        for cell in cells
    )
...
function within_correlation_matrix(data::DataFrame)
    ...
    isempty(within_factors) && throw(
        ArgumentError(
            "No within-subjects factors found. within_correlation_matrix requires at least one within-subjects factor.",
        ),
    )

    # Convert to wide format: create combined condition column from within-subjects factors only
    data_wide =
        length(within_factors) > 1 ?
        transform(
            data,
            within_factors =>
                ByRow((args...) -> join([string(a) for a in args], "_")) => :condition,
