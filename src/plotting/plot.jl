...
- `plot_type`: Type of plot to create. Options: `:line` (default), `:bar`, `:violin`, `:boxplot`, `:raincloud`, `:raincloud_custom`, or `:raincloud_custom_2x2`. Note: `:violin`, `:boxplot`, and `:raincloud` variants require raw data in `result.data`. A raincloud plot combines a violin plot, boxplot, and individual data points (individual points are always shown in raincloud plots via Makie's built-in `rainclouds!`).
- `errorbars`: Type of error bars. Options: `:SE` (default, standard error), `:SD` (standard deviation), `:CI` (confidence interval), `:withinSE` (within-subjects SE, Cousineau-Morey method), `:withinCI` (within-subjects CI), or `:none` (no error bars). `:withinSE` and `:withinCI` require raw data in `result.data`.
